...
- Per ogni intersezione, ad ogni `step()` si aggiorna il **massimo storico** (mai un valore istantaneo o una media) del tempo di attesa osservato tra tutti i veicoli presenti sulle corsie di quel gruppo, usando `get_lane_vehicles()` (corsia intera, nessun cutoff di visibilità — a differenza di `wait_vec` dello stato RL) incrociato con `vehicle_wait_times`. `vehicle_wait_times` è cumulativo per tutto il tempo in cui il veicolo resta "in ingresso" a un'intersezione (si azzera solo quando la attraversa), **non** un'attesa sull'intero viaggio — un veicolo può attraversare più incroci, l'attesa riparte da zero ad ognuno. Corretto il 14/9/2026: prima si azzerava appena il veicolo aveva velocità ≥0.1 m/s in un singolo istante, anche se si era solo spostato in avanti nella coda senza attraversare — sottostimava sistematicamente questi massimi (verificato empiricamente: mediana 45s persi per reset spurio, fino a 330s, su un singolo episodio di test prima della correzione). Ogni numero di `max_wait_ns`/`max_wait_ew` calcolato prima di questa data è quindi una sottostima, non solo di `vehicle_wait_times` — vedi `descrizione_stato_meta_reward.md` §10.
