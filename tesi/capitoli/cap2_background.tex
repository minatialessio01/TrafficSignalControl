...
($\hat D_{ii} = \sum_j \hat A_{ij}$), la regola di propagazione di un
...
vicino $j$ con $1/\sqrt{\hat d_i \hat d_j}$:
\begin{equation}
  h_i' = \sigma\!\left(\sum_{j \in \mathcal{N}(i) \cup \{i\}}
    \frac{1}{\sqrt{\hat d_i \hat d_j}}\, W h_j\right)
...
$\alpha_{ij}$ consente a un nodo di ponderare diversamente i propri vicini
...
alla previsione: un layer di attenzione grafica cattura la dipendenza
...
MetaSTGAT applichi questo principio sia al layer di attenzione grafica
...
quali attenzione grafica, una componente ricorrente sul tempo e reinforcement
...
affianca all'attenzione grafica (GAT) un componente ricorrente (LSTM) per catturare la dipendenza temporale oltre
