...
- **`dqn_agent.py`** — `DQNAgent` avvolge sia `MetaSTGAT` che `STGAT` (rilevato con `isinstance(model, MetaSTGAT)`) in un agente DQN multi-intersezione con **parameter sharing** (un solo modello per tutte le intersezioni, batch sulla dimensione N). Punti chiave:
  ...
  - Tutti i flag `use_*` del costruttore sono i sette meccanismi di ablation lato agente (Double DQN, PER, Huber, soft update, grad clip, BPTT, Tanh meta) — vedi tabella in `metastgat_diff_analysis.md`/`descrizione_modelli.md` §1 per la mappa completa M1-M10.
...
- I flag di ablation non sono sparsi arbitrariamente: sono organizzati in **preset** (`--ablation pro/paper/environment/temporal/rl_core/replay_stability`, definiti in `scripts/train.py`) che corrispondono al raggruppamento per sottosistemi funzionali — vedi `descrizione_modelli.md` §3 per la razionale scientifica e il confronto con le 3 alternative scartate.
