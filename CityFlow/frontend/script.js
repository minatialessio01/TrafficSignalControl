...
function uploadFileUrl(v, url, callback) {
    infoAppend("Fetching " + url);
    fetch(url)
        .then(response => response.text())
        .then(text => {
            v[0] = text;
            infoAppend(url + " loaded");
            callback();
        })
        .catch(err => {
            infoAppend("Fetching failed");
            console.error(err);
        });
}
...
function start() {
    ...
    if (RoadnetFileDom.files.length > 0 && ReplayFileDom.files.length > 0) {
        uploadFile(roadnetData, RoadnetFileDom.files[0], function() {
            uploadFile(replayData, ReplayFileDom.files[0], processFiles);
        });
    } else {
        uploadFileUrl(roadnetData, "./roadnet.log", function(){
            uploadFileUrl(replayData, "./replay.log", processFiles);
        });
    }
}
