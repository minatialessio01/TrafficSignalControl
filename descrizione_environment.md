...
- **Reward** `r_i^t`, due modalità (`reward_mode`):
  - **`custom`** (avanzata): `reward = clip((Passed - Incoming - alpha*max_red_wait_time - wasted_green_penalty) / 100, [-3, 0])` — multi-obiettivo (throughput, coda sulle corsie rosse, penalità se un verde non fa passare nessuno). Clip corretto tre volte: 14/9/2026 `[-20,5]→[-10,0]` (`Passed <= Incoming` sempre per costruzione, il lato `+5` non era mai raggiungibile); 15/9/2026 `[-10,0]→[-4,0]` dopo la correzione del tempo di attesa (`descrizione_stato_meta_reward.md` §10); stesso giorno `[-4,0]→[-3,0]` con lo sweep su alpha fissato a `{0.08, 0.04, 0.00}` (sostituisce `{0.2, 0.1, 0.0}` — vedi `descrizione_modelli.md` §2): il worst case fisico con alpha=0.08 è `-3.50`, leggermente oltre `-3` — il clip tocca (di poco) anche il caso limite assoluto, voluto per renderlo un vincolo reale invece che decorativo — vedi la nota nel docstring di `_compute_rewards` in `cityflow_env.py` per il calcolo completo.
